...
tbl = pd.DataFrame(columns=['itemId','name','upc','categoryPath','shortDescription','longDescription',
                   'brandName','thumbnailImage','mediumImage','largeImage','size',
                   'color','modelNumber','productUrl','customerRating',
                   'numReviews','offerType','datasheet_url'])  
...
for i, url in enumerate(urls):
    ...
    soup = BeautifulSoup(data)
    try:
#        soup.find("div", {"class": "brandModelInfo"})
        model = soup.find("h2", {"class": "product_details modelNo"})
        model = model.text.split('#')[-1].strip() if model else ''
        brand_label = soup.find("h2", {"class": "product-title__brand"})
        brand = brand_label.text.strip() if brand_label else ''
